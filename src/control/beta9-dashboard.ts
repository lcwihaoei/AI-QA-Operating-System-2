...
function closeDialog(backdrop,value){if(!backdrop||!backdrop.parentNode)return;backdrop.remove();document.body.classList.remove('beta9-dialog-open');const handler=backdrop.__keyHandler;if(handler)document.removeEventListener('keydown',handler);const resolve=backdrop.__resolve;if(resolve)resolve(value)}
function governedDialog(options){return new Promise(resolve=>{const backdrop=document.createElement('div');backdrop.className='beta9-dialog-backdrop';backdrop.setAttribute('role','presentation');backdrop.__resolve=resolve;const inputHtml=options.input?'<label class="beta9-dialog-field"><span>'+esc(options.input.label)+'</span><input id="beta9DialogInput" autocomplete="off" maxlength="'+esc(options.input.maxLength||120)+'" value="'+esc(options.input.value||'')+'" placeholder="'+esc(options.input.placeholder||'')+'"></label>':'';const callout=options.callout?'<div class="beta9-dialog-callout">'+options.callout+'</div>':'';const list=options.items&&options.items.length?'<ul class="beta9-dialog-list">'+options.items.map(v=>'<li>'+esc(v)+'</li>').join('')+'</ul>':'';backdrop.innerHTML='<section class="beta9-dialog" role="dialog" aria-modal="true" aria-labelledby="beta9DialogTitle"><div class="beta9-dialog-header"><div class="beta9-dialog-icon">✦</div><div><h2 id="beta9DialogTitle">'+esc(options.title)+'</h2><p>'+esc(options.message||'')+'</p></div><button type="button" class="beta9-dialog-close" data-dialog-cancel aria-label="'+esc(L('Close','關閉'))+'">×</button></div><div class="beta9-dialog-body">'+callout+list+inputHtml+'</div><div class="beta9-dialog-actions"><button type="button" class="secondary-button" data-dialog-cancel>'+esc(options.cancelLabel||L('Cancel','取消'))+'</button><button type="button" class="'+esc(options.confirmClass||'primary-button')+'" data-dialog-confirm>'+esc(options.confirmLabel||L('Continue','繼續'))+'</button></div></section>';document.body.appendChild(backdrop);document.body.classList.add('beta9-dialog-open');const input=backdrop.querySelector('#beta9DialogInput');const done=ok=>{if(!ok)return closeDialog(backdrop,null);const value=input?input.value.trim():true;if(options.input&&options.input.required&&!value){input.focus();return}closeDialog(backdrop,value)};backdrop.querySelectorAll('[data-dialog-cancel]').forEach(el=>el.addEventListener('click',()=>done(false)));backdrop.querySelector('[data-dialog-confirm]').addEventListener('click',()=>done(true));backdrop.addEventListener('click',event=>{if(event.target===backdrop)done(false)});const keyHandler=event=>{if(event.key==='Escape')done(false);if(event.key==='Enter'&&input&&document.activeElement===input){event.preventDefault();done(true)}};backdrop.__keyHandler=keyHandler;document.addEventListener('keydown',keyHandler);setTimeout(()=>{(input||backdrop.querySelector('[data-dialog-confirm]')).focus()},0)})}
async function notice(message){await governedDialog({title:L('Action needs attention','動作需要注意'),message:String(message||''),confirmLabel:L('Close','關閉'),cancelLabel:L('Close','關閉')})}
...
async function runAction(action,itemId){
  ...
  try{
    if(action==='plan')await post('/api/beta9/plan',{itemId});
      ...
      if(!a.plan)throw new Error(L('No reviewed plan is available.','沒有可審閱的修復方案。'));
      const files=(a.plan.changes||[]).map(change=>change.operation+' · '+change.path);
      const who=await governedDialog({title:L('Approve exact reviewed source scope','核准已審閱的精確程式碼範圍'),message:L('Approval is bound to the exact files in this reviewed plan. A changed plan will require a new approval.','此核准只綁定目前修復方案中的精確檔案；方案一旦變更就必須重新核准。'),callout:'<strong>'+esc(L('Plan hash','計畫雜湊'))+'</strong><code class="beta9-dialog-hash">'+esc(a.plan.planHash)+'</code>',items:files,input:{label:L('Approver','核准者'),value:'owner',maxLength:120,required:true},confirmLabel:L('Approve exact files','核准精確檔案')});
      if(!who)return;await post('/api/beta9/approve',{itemId,planHash:a.plan.planHash,approvedBy:who});
    }else if(action==='execute'){
      if(!a.plan)throw new Error(L('No approved plan is available.','沒有已核准的修復方案。'));const tail=a.plan.planHash.slice(-8);
      const typed=await governedDialog({title:L('Confirm bounded source mutation','確認受控程式碼修改'),message:L('This will modify only the approved files on an isolated branch, then run targeted tests, regression and Beta.7 QA. It does not commit, push or merge.','此動作只會在隔離分支修改已核准檔案，接著執行目標測試、回歸測試與 Beta.7 QA；不會 commit、push 或 merge。'),callout:'<strong class="beta9-dialog-danger">'+esc(L('Write confirmation required','需要寫入確認'))+'</strong><small>'+esc(L('Type the last 8 characters of the reviewed plan hash: ','請輸入已審閱 plan hash 的最後 8 碼：'))+'<b class="beta9-dialog-hash">'+esc(tail)+'</b></small>',items:(a.plan.changes||[]).map(change=>change.operation+' · '+change.path),input:{label:L('Plan hash confirmation','Plan hash 確認'),placeholder:tail,maxLength:8,required:true},confirmLabel:L('Execute approved fix','執行已核准修復')});
      if(typed!==tail){if(typed)await notice(L('The confirmation did not match the reviewed plan hash. No files were changed.','確認碼與已審閱 plan hash 不符，未修改任何檔案。'));return}await post('/api/beta9/execute',{itemId,planHash:a.plan.planHash,confirmWrite:true});
    }else if(action==='correlate'){
      const ok=await governedDialog({title:L('Correlate fresh Beta.7 evidence','比對最新 Beta.7 證據'),message:L('The server will use the explicitly configured post-fix result or accept exactly one unambiguous fresh run produced after this attempt started. Multiple candidates are blocked instead of guessed.','伺服器會使用明確指定的修復後結果，或只接受本次嘗試開始後唯一且無歧義的新 QA run；若有多個候選會阻擋，不會猜測。'),confirmLabel:L('Correlate now','立即比對')});
      if(!ok)return;await post('/api/beta9/correlate',{itemId});
    }else if(action==='retry'){
      const ok=await governedDialog({title:L('Prepare a bounded retry','準備受控重試'),message:L('Unresolved isolated changes will be rolled back. The retry authorization uses fresh QA evidence and requires a completely new fix plan plus a new source-scope approval.','未解決的隔離變更會先回滾；重試授權以最新 QA 證據為依據，且必須重新產生修復方案並再次核准程式碼範圍。'),confirmLabel:L('Prepare retry','準備重試')});
      if(!ok)return;await post('/api/beta9/prepare-retry',{itemId});
    }
  }catch(e){await notice(String(e&&e.message||e));}finally{actionPending=false;await refreshAllBeta9();}
}
async function createSelection(){
  ...
  if(!fingerprints.length){await notice(L('Select at least one finding.','請至少選擇一個問題。'));return;}
  const ok=await governedDialog({title:L('Create Beta.9 review plan','建立 Beta.9 審閱計畫'),message:L('Create a review plan for ','要為 ')+fingerprints.length+L(' selected finding(s). This step remains read-only and does not modify product source code.',' 個已選問題建立審閱計畫。這一步仍為唯讀，不會修改產品程式碼。'),items:fingerprints.slice(0,20),confirmLabel:L('Create review plan','建立審閱計畫')});
  if(!ok)return;
  try{await post('/api/beta9/select',{fingerprints});await refreshAllBeta9();}catch(e){await notice(String(e&&e.message||e));}
}
