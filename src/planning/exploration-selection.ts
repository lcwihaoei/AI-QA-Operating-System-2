 ...
 *
 * Field validation showed that reserving at most three interactions per page
 * leaves complex frontends at only a few percent interaction coverage. The
 * reliability profile keeps BFS route diversity, but lets deterministic safe
 * interactions consume up to 45% of the per-page candidate budget (capped at
 * eight). Risk decisions are still made before this selector, so increasing
 * coverage never unlocks blocked/destructive actions.
 */
